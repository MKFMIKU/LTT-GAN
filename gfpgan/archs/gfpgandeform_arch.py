...
from basicsr.ops.dcn import DeformConvPack
...
class GFPGANDEFORM(nn.Module):
    ...
    def __init__(
        ...
        super(GFPGANDEFORM, self).__init__()
        ...
        for i in range(self.log_size, 2, -1):
            ...
            self.conv_body_down.append(ResDBlock(in_channels, out_channels, resample_kernel))
...
class ResDBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, resample_kernel=(1, 3, 3, 1)):
        ...
        self.convd = DCNv2Pack(out_channels, out_channels, 3, stride=1, padding=1)
        self.convd_act = FusedLeakyReLU(out_channels)

    def forward(self, x):
        out = self.conv1(x)
        out = self.conv2(out)
        skip = self.skip(x)
        out = self.convd_act(self.convd(out, skip)) + skip
        out = out / math.sqrt(2)
        return out
