...
def estimate_aggd_params(x):
    x_left = x[x < 0]
    x_right = x[x >= 0]
    size_left = x_left.size
    size_right = x_right.size
    stddev_left = math.sqrt((1.0 / (size_left - 1)) * numpy.sum(x_left ** 2))
    stddev_right = math.sqrt((1.0 / (size_right - 1)) * numpy.sum(x_right ** 2))
    if stddev_right == 0:
